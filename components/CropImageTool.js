...
export default function CropImageTool({ lang = "en" }) {
  ...
  function down(e){ if(!src) return; const p=point(e); setDragging(true); setStart(p); setRect({x:p.x,y:p.y,w:0,h:0}); }
  function move(e){ if(!dragging||!start) return; e.preventDefault(); const p=point(e); setRect({x:Math.min(start.x,p.x), y:Math.min(start.y,p.y), w:Math.abs(p.x-start.x), h:Math.abs(p.y-start.y)}); }
  function up(){ setDragging(false); }
  async function download(){
    const img=imgRef.current; if(!img||!rect||rect.w<2||rect.h<2) return;
